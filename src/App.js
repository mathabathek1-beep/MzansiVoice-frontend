...
export default function App() {
  ...
  const handleVote = async (choice) => {
    if (!poll) return;
    try {
      await axios.post(`${API}/vote`, { pollId: poll.id, choice });
        ...
        <div>
          ...
          {adminPolls.map(p => (
            <div key={p.id} style={{ border: '1px solid #ccc', margin: 10, padding: 10 }}>
              ...
              <button onClick={() => exportPollResults(p.id)} style={{ background: 'blue', color: 'white', marginRight: 10 }}>Export CSV</button>
              <button onClick={() => handleDeletePoll(p.id)} style={{ background: 'red', color: 'white' }}>Delete</button>
            </div>
          ))}
        </div>
      )}
